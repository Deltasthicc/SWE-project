...
public class InventoryPanel extends JPanel {
        ...
        new SwingWorker<List<Book>, Void>() {
            ...
            @Override protected void done() {
                try {
                    List<Book> books = get();
                    if (books.isEmpty()) {
                        String err = bas.db.BookCache.getInstance().getLastError();
                        if (err != null) { status("Warning: " + err); return; }
                    }
                    model.setRowCount(0);
    ...
    // FIXED: Pattern.quote prevents regex crash on special chars like [ ( \ etc.
    private void applyFilter() {
        String t = filterField.getText().trim();
        sorter.setRowFilter(t.isEmpty() ? null : RowFilter.regexFilter("(?i)" + Pattern.quote(t)));
    }
    ...
    private void openStockUpdate() {
        ...
        try {
            ...
            if (actor == null) actor = managerId;
            if (DatabaseManager.getInstance().addStock(isbn, qty, actor)) {
                status("Stock updated: +" + qty + " for " + title); load();
    ...
    private void openAddDialog() {
        ...
        JTextField isbnF=new JTextField(18), titleF=new JTextField(22), authF=new JTextField(22),
            pubF=new JTextField(22), paddrF=new JTextField(22), priceF=new JTextField("0.00",10),
            rackF=new JTextField(8), stockF=new JTextField("0",6), threshF=new JTextField("5",6),
            leadF=new JTextField("2",6), wkSalF=new JTextField("0.0",6);
        ...
        save.addActionListener(e -> {
            try {
                ...
                Book b = new Book(ISBNValidator.clean(isbnF.getText()),
                    titleF.getText().trim(), authF.getText().trim(),
                    pubF.getText().trim(), paddrF.getText().trim(),
                    Double.parseDouble(priceF.getText().trim()), rackF.getText().trim(),
    ...
    private void openEdit() {
        ...
        JTextField titleF=new JTextField(b.getTitle(),22), authF=new JTextField(b.getAuthor(),22),
            pubF=new JTextField(b.getPublisher(),22), paddrF=new JTextField(nullSafe(b.getPublisherAddress()),22),
            priceF=new JTextField(String.format("%.2f",b.getUnitPrice()),10),
            rackF=new JTextField(nullSafe(b.getRackLocation()),8),
            stockF=new JTextField(String.valueOf(b.getStockCount()),6),
            threshF=new JTextField(String.valueOf(b.getRestockThreshold()),6),
            leadF=new JTextField(String.valueOf(b.getProcurementLeadTimeWeeks()),6),
            wkSalF=new JTextField(String.format("%.1f",b.getWeeklySales()),6);
    ...
    // FIXED: Selective notification — only mark successfully sent emails
    private void sendAlerts() {
        ...
        List<String> sent = EmailService.sendBulkAlertsSelective(emails, title, isbn);
        for (String e : sent) DatabaseManager.getInstance().markNotifiedByEmail(isbn, e);
        status("Sent " + sent.size() + "/" + emails.size() + " restock alert(s)." +
            (sent.size() < emails.size() ? " Failed emails remain PENDING." : ""));
    }
    ...
    private JPanel formPanel(String[] labels, JTextField[] fields) {
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            JLabel l = new JLabel(labels[i]); l.setFont(new Font("SansSerif",Font.PLAIN,12));
            p.add(l,g); g.gridx=1; g.weightx=1; p.add(fields[i],g);
        }
        return p;
    }
    private JPanel btnsPanel(JButton cancel, JButton save) {
        JPanel p = new JPanel(new FlowLayout(FlowLayout.RIGHT)); p.add(cancel); p.add(save); return p;
    }
    private JLabel errLabel() {
        JLabel l = new JLabel(" ", SwingConstants.CENTER);
        l.setForeground(new Color(220,38,38)); l.setFont(new Font("SansSerif",Font.PLAIN,11)); return l;
    }
    private JButton btn(String txt, Color bg, Color fg) {
        JButton b = new JButton(txt); b.setBackground(bg); b.setForeground(fg);
        b.setFocusPainted(false); b.setOpaque(true); b.setBorderPainted(false); return b;
    }
    ...
    private int intVal(Object o) { try{return Integer.parseInt(o.toString());}catch(Exception e){return 0;} }
